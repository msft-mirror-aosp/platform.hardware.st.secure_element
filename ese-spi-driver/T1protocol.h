 ...
 */
char T1protocol_getValidPcb(
        TpduType type,
        RBlockType subtype,
        uint8_t numSeqMaster,
        uint8_t numseqSlave,
        bool isLast);
 ...
 * @return 0 If checksum is ok, -1 otherwise.
 */
int T1protocol_checkSBlockResponseConsistency(
        Tpdu *lastCmdTpduSent,
 ...
 */
int T1protocol_checkTpduConsistency(
        Tpdu *lastCmdTpduSent,
        Tpdu *lastRespTpduReceived);
 ...
 * @return 0.
 */
int T1protocol_processIBlock(
        Tpdu *originalCmdTpdu);
 ...
 */
int T1protocol_doResendRequest(
        Tpdu *lastCmdTpduSent,
        Tpdu *lastRespTpduReceived,
        int *bytesRead);
 ...
 * @return 0 if everything went fine, -1 if something failed.
 */
int T1protocol_doResyncRequest(
        Tpdu *lastCmdTpduSent,
        Tpdu *lastRespTpduReceived,
        int *bytesRead);
 ...
 */
int T1protocol_handleTpduResponse(
        Tpdu *originalCmdTpdu,
        Tpdu *lastCmdTpduSent,
        Tpdu *lastRespTpduReceived,
        int *bytesRead);
...
int T1protocol_transcieveApduPart(char *cmdApduPart, uint8_t cmdLength,
        bool isLast, char *respApduPart, uint8_t *respLength);
